...
const VIEWS = {
  ...
  unable: {
    title: "Unable to do it",
    desc: "applications the automation couldn't complete — the reason + what it saw",
    render() {
      const items = state.apps.filter((a) =>
        ["failed", "error"].includes(a.status)
        || (a.status === "needs_human" && a.gate_reason === "unknown_field"));
      if (!items.length) return `<div class="panel"><div class="empty">
        Nothing here — nothing has failed to complete.</div></div>`;
      items.sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at));
      const cards = items.map((r) => {
        const reason = r.closed_reason || r.gate_question
          || "Couldn't complete the application.";
        const shot = r.screenshot_url
          ? `<a class="unable-shot" href="${esc(r.screenshot_url)}" target="_blank" rel="noopener">
               <img src="${esc(r.screenshot_url)}" alt="what the browser saw" loading="lazy"></a>`
          : `<div class="unable-shot none">no screenshot</div>`;
        return `<div class="panel unable-card">
          <div class="unable-body">
            <div class="unable-head">
              <div class="t-co" style="font-size:15px">${esc(r.company)}
                <span style="color:var(--faint);font-weight:400"> — ${esc(r.title)}</span></div>
              <button class="btn" data-open="${esc(r.pk)}">Open</button>
            </div>
            <div class="unable-reason">${esc(reason)}</div>
            <div class="mono" style="font-size:11px;color:var(--faint);margin-top:auto;padding-top:8px">
              ${r.discovered_at ? "found " + when(r.discovered_at) + " · " : ""}${esc(r.pk)}${r.jd_url
                ? ` · <a href="${esc(r.jd_url)}" target="_blank" rel="noopener" style="color:var(--muted-fg)">view posting ↗</a>` : ""}
            </div>
          </div>
          ${shot}
        </div>`;
      }).join("");
      return `<div class="unable-list">${cards}</div>`;
    },
  },
...
function cardHtml(r) {
  ...
  const bits = [
    r.discovered_at && `found ${when(r.discovered_at)}`,
  ...
  // A job stuck in "Working now" (tailored / submitting) can be pulled out —
  // cancel an in-progress apply, or skip one that hasn't started.
  const working = ["tailored", "submitting"].includes(r.status);
  const cancelBtn = working
    ? `<button class="card-cancel" data-cancel="${esc(r.pk)}"
         title="Move to Closed">✕ ${r.status === "submitting" ? "cancel" : "skip"}</button>`
    : "";
  const links = jobUrl || cancelBtn
    ? `<div class="card-links">${jobUrl}${cancelBtn}</div>` : "";
...
function wire() {
  ...
  $("#view").addEventListener("click", (e) => {
    ...
    if (cancel) {
      e.stopPropagation();
      const pk = cancel.dataset.cancel;
      const st = (state.apps.find((a) => a.pk === pk) || {}).status;
      const inflight = st === "submitting";
      if (!confirm(`${inflight ? "Cancel this in-progress application"
        : "Skip this job"}? It moves to Closed.`)) return;
      post(`/actions/skip/${encodeURIComponent(pk)}`);
      toast(inflight ? "cancelled — moved to Closed" : "skipped");
      return;
    }
    if (approveAll) {
