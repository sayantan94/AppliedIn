...
async def crawl(url: str, company: str, search_terms: list[str], model: str) -> list[dict]:
    ...
    task = (
        ...
        "  2. if the results can be sorted, sort by DATE POSTED / most-recent first,\n"
        "  3. extract the postings on the FIRST page of results — job_id from the URL, "
        "title, url. Only keep roles posted in the LAST ~15 DAYS: if a posting shows a "
        "date or age (e.g. 'Posted 3 weeks ago') older than ~15 days, SKIP it.\n"
        "  4. then move on to the next term.\n"
        "Do NOT page beyond the first results page for a term, do NOT keep scrolling, "
        "do NOT log in or apply.\n"
        "When you've searched all the terms (or gathered ~25 recent postings total), "
        "FINISH and return the combined, de-duplicated postings as structured output — "
        "each with job_id (stable id from the URL), title, and url (direct link). Prefer "
        "finishing over gathering more."
    )
